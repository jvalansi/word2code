...
class RaiseThisBarn:
    def calc(self, str):
        ...
        def parts(possibility):
            #### return (str[:k], str[k:])
            return (str[:possibility], str[possibility:])
        ...
        def valid(possibility):
            #### return same(*list(contain(part, cows) for part in parts(k))) 
            return eq(*list(countOf(part, types['cows']) for part in parts(possibility))) 
        ...
        return len(filter(valid, possibilities))

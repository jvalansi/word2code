...
class TheBrickTowerEasyDivTwo:
    def find(self, redCount, redHeight, blueCount, blueHeight):
        ...
        def valid(possibility):
            #### reduce = lambda possibility: (tower and no(list(possibility)))
            reduce = (lambda possibility0: (possibility and not_(list(possibility0))))
            #### mapping = lambda bricks: bricks
            mapping = (lambda bricks: bricks)
            #### possibilities = two(tower)
            possibilities = cpairs(possibility)
            #### valid = lambda bricks: same(*bricks)
            valid = (lambda bricks: eq(*bricks))

...
class GogoXBallsAndBinsEasy:
	def solve(self, T):
		...
		# More precisely, he wanted to reach a state with T[0] balls in bin 0, T[1] balls in bin 1, and so on, such that the following two conditions are met:
		...
		mapping = lambda possibility: sum(abs(possibility[i] - input_array[i]) for i in range(N))/2   
